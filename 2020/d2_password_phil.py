...
def parse_pass(password: str):
    arr = password.split()
    bounds = arr[0].split('-')
    lowerBound, upperBound = int(bounds[0]), int(bounds[1])
    letter = arr[1][:-1]
    word = arr[2]

    return lowerBound, upperBound, letter, word

#print(parse_pass("10-12 j: jjjjjjjjjzjjdj"))

def pass_phil(text):
    valid = 0
    for n in text:
        lowerBound, upperBound, letter, word = parse_pass(n)
        amount = word.count(letter)
        if amount <= upperBound and amount >= lowerBound:
            valid += 1
    return valid

print(pass_phil(text))

def pass_phil2(text):
    valid = 0
    for n in text:
        first, second, letter, word = parse_pass(n)
        if word[first - 1] == letter and word[second - 1] == letter:
            continue
        elif word[first - 1] == letter or word[second - 1] == letter:
            valid += 1
    
    return valid

print(pass_phil2(text))
